...
// Wallet del Owner (debe tener tokens previamente minteados)
...
async function main() {
  ...
  const amount = ethers.parseUnits("100000", decimals); // 🔥 quema inicial: 100,000 DOA
...
main().catch((err) => {
  console.error("❌ Error en burnByTransferOwner:", err);
  process.exitCode = 1;
});
