...
function Distribuir-Recompensas($dia, $categoria, $mensaje) {
    try {
        ...
        if ($res -is [PSCustomObject] -and $res.txHash) { return $res.txHash } else { return "ERROR: salida inválida de recompensas.ps1" }
    } catch { return "ERROR: $($_.Exception.Message)" }
}

# --- Ejecutar publicaciones del día ---
$hoy = (Get-Date).DayOfYear   # más robusto que .Day
foreach ($t in $tareas) {
    if (-not $t.PSObject.Properties["Dia"] -or -not $t.PSObject.Properties["Mensaje"]) {
        Write-Host "⚠️ Entrada inválida en calendario: $($t | ConvertTo-Json -Compress)" -ForegroundColor Yellow
        continue
    }
