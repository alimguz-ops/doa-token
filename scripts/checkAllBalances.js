...
// Guarda resultados en logs/balances.csv y logs/balances.json (histórico)
...
// ✅ Función robusta para consultar balances
async function getTokenBalance(provider, tokenInfo, addr) {
  try {
    ...
    console.error(`⚠️ Error consultando balance de ${tokenInfo.address} en ${addr}: ${err.shortMessage || err.message}`);
...
async function main() {
  ...
  for (const [name, addr] of Object.entries(addresses)) {
    try {
      ...
      console.error(`❌ Error procesando cuenta ${name} (${addr}): ${err.message}`);
    }
  }

  // Guardar CSV en logs/
  const csvPath = `${logsDir}/balances.csv`;
  fs.writeFileSync(csvPath, csvContent);
  console.log(`✅ Resultados guardados en ${csvPath}`);

  // Guardar histórico JSON en logs/
  const jsonPath = `${logsDir}/balances.json`;
  let history = [];
...
main().catch((err) => {
  console.error("❌ Error general en auditoría:", err.message);
  process.exitCode = 1;
});
