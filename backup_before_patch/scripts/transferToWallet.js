...
async function main() {
  ...
  // Signer: debe ser la cuenta configurada en hardhat.config.js
  const [sender] = await ethers.getSigners();

  // Wallet MetaMask destino
  const recipient = "0xf224bc9a97e0e605c0546f9ced88aaf2228cf6c5";

  if (!ethers.isAddress(doaTokenAddress) || !ethers.isAddress(recipient)) {
  ...
  console.log(`✅ Transferidos ${ethers.formatUnits(amount, 18)} DOA a ${recipient} en bloque ${receipt.blockNumber}, gas usado: ${receipt.gasUsed}`);
