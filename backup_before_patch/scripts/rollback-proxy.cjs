...
async function main() {
  ...
  if (!ethers.isAddress(proxyAddress) || !ethers.isAddress(oldImplAddress) || !ethers.isAddress(proxyAdminAddress)) {
    throw new Error("âŒ Alguna direcciÃ³n no es vÃ¡lida");
  }
  ...
  console.log(`âœ… Proxy apuntado de nuevo a la implementaciÃ³n anterior ${oldImplAddress} en bloque ${receipt.blockNumber}, gas usado: ${receipt.gasUsed}`);
}

main().catch((error) => {
  console.error("âŒ Error en rollback proxy:", error.message);
  process.exit(1);
});
