...
async function main() {
  ...
  const amountIn = ethers.parseEther("0.1"); // 0.1 MATIC
  const pathBuy = [WETH_ADDRESS, TOKEN_ADDRESS];
  const deadline = Math.floor(Date.now() / 1000) + 600;

  try {
    const txBuy = await router.swapExactETHForTokens(
      0, // mÃ­nimo de tokens aceptados (ajusta con slippage)
      pathBuy,
      signer.address,
      deadline,
      { value: amountIn }
    ...
    console.log(`âœ… Compra realizada con 0.1 MATIC â†’ Token en bloque ${receiptBuy.blockNumber}, gas usado: ${receiptBuy.gasUsed}`);
  } catch (e) {
  ...
  if (balance > 0n) {
    try {
      ...
      const txSell = await router.swapExactTokensForETH(
        balance,
        0, // mÃ­nimo de MATIC aceptado (ajusta con slippage)
        pathSell,
        signer.address,
        deadline
      );
      const receiptSell = await txSell.wait();
      console.log(`âœ… Venta realizada Token â†’ MATIC en bloque ${receiptSell.blockNumber}, gas usado: ${receiptSell.gasUsed}`);
    } catch (e) {
...
main().catch((err) => {
  console.error("âŒ Error general:", err.message);
  process.exitCode = 1;
});
