...
async function main() {
  ...
  for (const d of history) {
    accTokens += Number(d.liquidity?.token || 0);
    accBase += Number(d.liquidity?.base || 0);
    labels.push(new Date(d.timestamp).toISOString().split("T")[0]);
  ...
  const config = {
    ...
    options: {
      ...
      plugins: {
        title: { display: true, text: "Liquidez acumulada por despliegue" }
      }
    }
  };
  ...
  console.log(`âœ… GrÃ¡fico acumulado generado en ${outputFile}`);
