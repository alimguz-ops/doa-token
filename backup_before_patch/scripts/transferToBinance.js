...
async function main() {
  if (!ethers.isAddress(tokenAddress) || !ethers.isAddress(binanceDepositAddress)) {
    throw new Error("❌ Dirección de token o depósito inválida");
  }
...
main().catch((err) => {
  console.error("❌ Error en transferToBinance:", err.message);
  process.exitCode = 1;
});