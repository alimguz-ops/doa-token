...
// Validar formato de dirección con ethers
...
try {
  ...
  if (Array.isArray(tokenlist.tokens) && tokenlist.tokens.length > 0) {
    ...
    console.log(`📂 Archivo actualizado: ${tokenlistPath}`);
  } else {
...
try {
  ...
  console.log(`📂 Archivo actualizado: ${deploymentsPath}`);
} catch (err) {
...
try {
  ...
  const regex = /Contrato \(Polygon\): .*/g;
  ...
  console.log(`📂 Archivo actualizado: ${readmePath}`);
} catch (err) {
