---
layout: default
title: TOKENOMICS
---

# 📊 TOKENOMICS – DOATokenV2 (DOA)  
# 📊 TOKENOMICS – DOATokenV2 (DOA)

---

## 1. Información general / General Information
- **Nombre del token / Token name:** DOATokenV2 (DOA)
- **Tipo / Type:** ERC-20 multichain (Ethereum + Polygon, chainId 1 y 137)
- **Contratos verificados / Verified contracts:**
  - [Ethereum – DOATokenV2](https://etherscan.io/address/0x6F52809EfdDF5826956EeF9C289A661624afb0cE)  
  - [Polygon – DOATokenV2](https://polygonscan.com/token/0x692d951163df3f7D9Fe071413F92c319D9B7369E)
- **Suministro máximo / Max supply:** 1,800,000 DOA
- **Precio inicial público / Initial public price:** USD 0.50 per DOA
- **Market Cap inicial (FDV):** USD 900,000

---

## 2. Distribución inicial / Initial Distribution
- **20% Liquidez inicial / Initial liquidity**  
  Reservado para crear los pares DOA/USDC y DOA/MATIC en QuickSwap, y DOA/WETH en Uniswap.  
- **30% Tesorería comunitaria / Community treasury**  
  Fondos para desarrollo, marketing y soporte de comunidad.  
- **25% Staking y recompensas / Staking & rewards**  
  Incentivos para holders y participantes en pools de liquidez.  
- **15% Gobernanza y desarrollo / Governance & development**  
  Para votaciones, mejoras técnicas y expansión del ecosistema.  
- **10% Reserva estratégica / Strategic reserve**  
  Uso futuro en alianzas, listados en CEX y expansión de integraciones DeFi.  

<p align="center">
  <img src="assets/Distribucion_DOATokenV2.png" alt="Distribución DOA Token V2 / DOA Token V2 Distribution" width="600"/>
</p>

---

## 3. Detalle de direcciones / Address Details
- **Owner:** 1,000,000 DOA – [0x6377cd174b35f3630b6d0db695f175d5f0dc5541](https://polygonscan.com/address/0x6377cd174b35f3630b6d0db695f175d5f0dc5541)  
- **Admin:** 200,000 DOA – [0xf224bc9a97e0e605c0546f9ced88aaf2228cf6c5](https://polygonscan.com/address/0xf224bc9a97e0e605c0546f9ced88aaf2228cf6c5)  
- **Reserva Binance / Binance Reserve:** 250,000 DOA – [0xfe7522c992ab5193ba66195ccbef65e3b0b76f95](https://polygonscan.com/address/0xfe7522c992ab5193ba66195ccbef65e3b0b76f95)  
- **Comunidad / Community:** 100,000 DOA – [0xD1f7a79CE44b267Dfe51B6F84008208550a30562](https://polygonscan.com/address/0xD1f7a79CE44b267Dfe51B6F84008208550a30562)  
- **Colaborador / Collaborator:** 250,000 DOA – [0xe3baefcbad73d05512deaad182ed0cf8b3a5e7b1](https://polygonscan.com/address/0xe3baefcbad73d05512deaad182ed0cf8b3a5e7b1)  
- **Wallet Binance:** [0xb27df745fc43ca79f8250e52639594ae96315ef5](https://polygonscan.com/address/0xb27df745fc43ca79f8250e52639594ae96315ef5)  

---

## 4. Objetivos / Objectives
- Garantizar liquidez inicial estable en Polygon y Ethereum.  
- Incentivar la participación comunitaria mediante staking y recompensas.  
- Financiar el crecimiento del ecosistema con la tesorería comunitaria.  
- Fortalecer la gobernanza DAO con fondos dedicados.  
- Reservar tokens estratégicos para alianzas y listados en CEX.  

---

## 5. Estado actual / Current Status
- Contratos verificados en Ethereum y Polygon.  
- Liquidez en preparación para QuickSwap y Uniswap.  
- Scripts automatizados para auditoría, gobernanza y liquidez.  
- Documentación modular en proceso: TOKENOMICS.md, ROADMAP.md, TEAM.md, AUDIT.md.  

---

## 6. Próximos pasos / Next Steps
- Activar liquidez en QuickSwap y Uniswap.  
- Publicar auditoría externa.  
- Consolidar comunidad DAO.  
- Completar aplicación QuickLaunch.  
- Cotización en CEX y expansión DeFi.  
